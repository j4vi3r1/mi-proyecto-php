...
// 1. SEGURIDAD: Recuperar el rol del usuario
$rol_usuario = $_SESSION['rol'] ?? 0; 

// 2. CAPTURAR PERIODO SELECCIONADO
...
<main class="min-h-screen bg-slate-50 py-12 px-6">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
                <span class="text-[#7c83e5] font-black uppercase text-xs tracking-widest leading-none">Módulo de Pagos</span>
                <h1 class="text-4xl font-black text-slate-800 italic tracking-tighter">Control de <span class="text-[#5be2f8]">Cobranza</span></h1>
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <form method="POST" onsubmit="return confirm('¿Seguro? Se borrarán TODOS los registros de este mes.');">
                    <input type="hidden" name="accion" value="limpiar_mes">
                    <button type="submit" class="p-4 bg-rose-50 text-rose-500 rounded-full hover:bg-rose-500 hover:text-white transition-all shadow-sm" title="Reiniciar Mes">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center gap-5">
                ...
                <div><p class="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1 leading-none">Monto Pendiente</p><h3 class="text-3xl font-black text-slate-800">$<?= number_format($total_pendiente, 0, ',', '.') ?></h3></div>
            </div>
            <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center gap-5">
                ...
                <div><p class="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1 leading-none">Monto Cobrado</p><h3 class="text-3xl font-black text-slate-800">$<?= number_format($total_pagado, 0, ',', '.') ?></h3></div>
            </div>
        </div>
        ...
        <?php if($faltan_cobros): ?>
            <div class="mb-8 bg-indigo-600 rounded-[2rem] p-6 shadow-xl shadow-indigo-200 flex flex-col md:flex-row items-center justify-between gap-4">
                <div class="flex items-center gap-4 text-white">
                    ...
                    <div>
                        <p class="font-black uppercase text-xs tracking-widest leading-none mb-1">Periodo Incompleto</p>
                        <p class="text-indigo-100 text-sm italic font-medium">Faltan <?= ($total_deberian - $ya_tienen) ?> contribuyentes.</p>
                    </div>
                </div>
                <form method="POST">
        ...
        <?php if(empty($cobros)): ?>
            <div class="bg-white rounded-[3rem] p-32 text-center border border-dashed border-slate-200 shadow-sm">
                <p class="text-slate-400 font-black uppercase text-[10px] tracking-[0.3em]">No hay registros generados para este mes</p>
            </div>
        <?php else: ?>
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-all group">
                            ...
                            <td class="px-8 py-6">
                                <p class="text-slate-800 font-black text-sm uppercase leading-tight italic"><?= htmlspecialchars($c['razon_social']) ?></p>
                            ...
                            <td class="px-8 py-6">
                                <div class="flex justify-center items-center gap-2">
                                    <button onclick="enviarWA('<?= $c['telefono'] ?>', '<?= $c['nombre_contacto'] ?>', '<?= $c['monto_a_cobrar'] ?>', '<?= $meses[$mes_sel] ?>')" class="p-3 bg-[#5be2f8]/10 text-[#2db5cc] rounded-xl hover:bg-[#5be2f8] hover:text-white transition-all active:scale-90 shadow-sm" title="Enviar Recordatorio">
                                    ...
                                    <form method="POST" action="marcar_pagado.php" onsubmit="return confirm('¿Eliminar registro?');">
                                        <input type="hidden" name="id_pago" value="<?= $c['id_pago'] ?>">
                                        <input type="hidden" name="mes" value="<?= $mes_sel ?>"><input type="hidden" name="anio" value="<?= $anio_sel ?>">
...
<div id="modalInd" class="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center hidden p-4">
    <div class="bg-white rounded-[2.5rem] p-10 max-w-md w-full shadow-2xl relative overflow-hidden">
        ...
        <h2 class="text-3xl font-black text-slate-800 mb-2 italic tracking-tighter">Cobro <span class="text-[#5be2f8]">Extra</span></h2>
        <p class="text-slate-500 text-xs mb-8 font-bold italic">Selecciona un cliente para el periodo <span class="text-[#7c83e5]"><?= $meses[$mes_sel] ?></span>.</p>
        
        <form method="POST">
            <input type="hidden" name="accion" value="generar_individual">
            <label class="text-[10px] uppercase font-black text-slate-400 tracking-[0.2em] ml-2">Contribuyente</label>
            <select name="rut_contribuyente" class="w-full bg-slate-50 border-none rounded-2xl p-4 mt-2 mb-8 font-bold text-slate-700 focus:ring-2 focus:ring-[#7c83e5] appearance-none">
                <?php foreach($todos_contri as $tc): ?>
                    <option value="<?= $tc['rut_contribuyente'] ?>"><?= $tc['razon_social'] ?></option>
                <?php endforeach; ?>
            </select>
            <div class="flex gap-4">
                <button type="button" onclick="document.getElementById('modalInd').classList.add('hidden')" class="flex-1 py-4 text-slate-400 font-black uppercase text-[10px] tracking-widest hover:text-slate-800 transition-colors">Cancelar</button>
...
function enviarWA(tel, nombre, monto, mesNombre) {
    ...
    const msj = `Hola ${nombre}, te saludamos de ArriendoPro. Recordamos el pago de honorarios de ${mesNombre} por un monto de ${montoFmt}. ¡Gracias!`;
